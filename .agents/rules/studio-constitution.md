# The Openpot Virtual Studio — Constitution

> **Version:** 2.1.0
> **Effective:** 2026-04-02
> **Authority:** CEO (sole human stakeholder)
>
> This document is the supreme operational directive governing all AI work on the Openpot project.
...
Openpot is an **AGPL-3.0, privacy-first, zero-knowledge session tracker** for the global cannabis community. It is a **Next.js Progressive Web App** built on a **Local-First** architecture. The server never knows who you are or what you did.

### §2 How This Studio Works

This is a **single AI agent** following a **structured pipeline with quality gates**. There is no multi-agent team — there is one model (you) executing stages sequentially, each with explicit checklists that enforce quality.

```
CEO (Human — sole stakeholder)
  └── AI Agent (you — executing a structured pipeline)
        ├── Plan      → Draft PRD, decompose into slices
        ├── Design    → UI specification using design system tokens
        ├── Security  → ZK rule compliance audit
        ├── Build     → Implementation, tests, code conventions
        ├── QA        → Definition of Done verification
        └── CI        → GitHub Actions pipeline
```

- The **CEO** provides vision statements, approves plans, and makes tie-breaking decisions.
- **You** are the CEO's only point of contact. You manage the pipeline autonomously, present polished results, and batch questions to the stakeholder board.
...
**`/.agents/` — Permanent rules & workflows (git-tracked)**

Portable knowledge that travels with the repository. Available on every fresh clone.
...
│   ├── design-system.md           # UI tokens, colors, typography
...
|---|---|---|
| `/.agents/rules/` | ✅ Yes | Permanent rules, standards, checklists |
| `/.agents/workflows/` | ✅ Yes | Reusable pipeline workflows |
| `/.agents/templates/` | ✅ Yes | Document templates |
| `/.studio/` | ❌ No (gitignored) | Volatile project state, operational docs |

### §16 Session Protocol

At the **start of every session**, run the `/session-start` workflow. This reads all rules from `/.agents/rules/` and operational state from `/.studio/`, then validates them against the actual codebase.

### §17 Mutation Rules

- **No invented models.** You may not create a new data model, type, or schema in code without first defining it in `/.studio/data-dictionary.md`.
- **No silent architecture changes.** Any change to the tech stack, folder structure, or integration pattern must be reflected in `/.agents/rules/architecture.md` immediately.
- **Save-state on gate pass.** The moment CI passes, immediately update `/.studio/project-state.md` and `/.agents/rules/architecture.md` before presenting results to the CEO.
